...
typedef enum
{
	...
	VMCALL_ACTION_SHADOW_USER,
	VMCALL_ACTION_SHADOW_KERNEL,
	VMCALL_ACTION_COUNT
} VMCALL_ACTION;
...
typedef struct _VM_PARAM_SHADOW_USER
{
	DWORD32 callerProcID;		/* IN */
	DWORD32 targetProcID;		/* IN */
	PUINT8 userTargetPageVA;	/* IN */
	PUINT8 userExecPageVA;		/* IN */
} VM_PARAM_SHADOW_USER, *PVM_PARAM_SHADOW_USER;

typedef struct _VM_PARAM_SHADOW_KERNEL
{
	UINT64 physTarget;
	PVOID payloadPage;
} VM_PARAM_SHADOW_KERNEL, *PVM_PARAM_SHADOW_KERNEL;
